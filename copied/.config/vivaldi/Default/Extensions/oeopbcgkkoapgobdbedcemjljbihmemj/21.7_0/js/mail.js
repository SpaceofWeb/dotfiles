...
class Mail {
    constructor() {
        ...
        this.markAsRead = function (params = {}) {
            var executeMailActionParams = shallowClone(params);
            // append these params
        ...
        this.postReply = function (message, replyAllFlag) {
            return that.account.executeMailAction({ mail: that, action: MailAction.REPLY, message: message, replyAllFlag: replyAllFlag });
        };
        ...
        this.getThread = async function (params = {}) {
            ...
            if (params.mail.messages || that.account.getAccountAddingMethod() == "oauth") {
                ...
                var responseWrapper = parseHtml(data);
                var tables = Array.from(responseWrapper.querySelectorAll(".maincontent .message"));
