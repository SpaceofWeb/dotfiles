...
function Account() {
   ...
   async function fetchFeed(params) {
        ...
        try {
            ...
            if (/AddMailService/i.test(responseUrl)) {
                ...
                if (responseUrl && responseUrl.indexOf("mail.google.com/mail/u/") == -1) {
            ...
            if (fetchEmailsResponse.feedsArrayInfo) {
                fetchEmailsResponse.feedsArrayInfo.forEach(feedInfo => {
                    ...
                    parsedFeed.querySelectorAll('entry').forEach(entry => {
                        ...
                        var authorName = entry.querySelector('author name').textContent;
                        var authorMail = entry.querySelector('author email').textContent;
                        var entryXML = new XMLSerializer().serializeToString(entry);
   ...
   async function executeGmailHttpAction(params) {
        ...
        if (params.action == MailAction.REPLY) {
            ...
            if (html && (html.indexOf("#FAD163") != -1 || html.toLowerCase().indexOf("your message has been sent") != -1)) {
                return "success";
            } else {
   ...
   this.getEmailsInAllLabels = function () {
        return emailsInAllLabels;
   }

   this.setEmailsInAllLabels = function (emails) {
        emailsInAllLabels = emails;
    }
   ...
   this.getLabels = async function(forceRefresh) {
        if (!labels || forceRefresh) {
            if (accountAddingMethod == "autoDetect") {
                const response = await fetchLabelsFromHtmlSource();
   ...
   this.remove = async function(oAuthForEmails, accounts) {
       if (accountAddingMethod == "autoDetect") {
           ...
           let tokenResponse = await oAuthForEmails.findTokenResponse({ userEmail: that.getEmail() });
           if (tokenResponse) {
