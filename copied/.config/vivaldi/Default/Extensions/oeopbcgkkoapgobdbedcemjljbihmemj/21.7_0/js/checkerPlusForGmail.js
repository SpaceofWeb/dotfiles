...
async function getAllEmails(params) {
    ...
    await asyncForEach(params.accounts, async account => {
...
async function initPopup(unreadCount) {
	...
	if (browserButtonAction == BROWSER_BUTTON_ACTION_CHECKER_PLUS_POPOUT) {
		...
		if (browserButtonAction == BROWSER_BUTTON_ACTION_GMAIL_TAB || browserButtonAction == BROWSER_BUTTON_ACTION_GMAIL_IN_NEW_TAB || browserButtonAction == BROWSER_BUTTON_ACTION_COMPOSE || checkerPlusElseCompose || checkerPlusElseGmailTab || gmailInboxElseCompose || gmailInboxElseGmailTab) {
			...
			var accountsSummary = await getAccountsSummary(bg.accounts);
			if (accountsSummary.signedIntoAccounts == 0) {
...
function getAccountByEmail(email) {
	...
	for (var a=0; a<accounts.length; a++) {
		if (accounts[a].getEmail().equalsIgnoreCase(email)) {
			return accounts[a];
		}
	}

	var ignoredAccounts = bg.ignoredAccounts;
	for (var a=0; a<ignoredAccounts.length; a++) {
		if (ignoredAccounts[a].getEmail().equalsIgnoreCase(email)) {
...
async function ensureContactsWrapper(accounts) {
    if (await storage.get("showContactPhoto")) {
        ...
        accountsWithUnreadMail.forEach(function(accountWithUnreadMail) {
            emailsWithUnreadMail.push(accountWithUnreadMail.getEmail());
        });
...
async function getContacts(params, callback) {
	...
	if (contactsData) {
		// maybe update
		if (params.account) {
			var dataIndex = getContactsDataIndexByEmail(contactsData, params.account.getEmail());
			if (dataIndex != -1) {
				callback({contacts:contactsData[dataIndex].contacts});
				//console.log("contacts from cache: " + params.account.getEmail());
			} else {
...
function generateContactPhotoURL(contact, account) {
	return new Promise((resolve, reject) => {
		if (contact.photoUrl) {
			oAuthForContacts.generateURL(account.getEmail(), contact.photoUrl).then(function (response) {
...
async function getFirstActiveEmail(accounts) {
	...
	if (firstActiveAccount) {
		return firstActiveAccount.getEmail();
	}
}
...
async function serializeOauthAccounts() {
	...
	const accountsToSerialize = [];
	bg.accounts.forEach(account => {
        accountsToSerialize.push({
            id: account.id,
...
async function resetSettings(accounts) {
	...
	accounts.forEach(function(account) {
		...
		var emailSetting = emailSettings[account.getEmail()];
		if (!emailSetting) {
			emailSetting = {};
		}
...
async function ajaxBasicHTMLGmail(url, options, secondAttempt) {
    ...
    if (response.ok) {
		...
		if (testGmailQuestion || (response.url && response.url.indexOf("v=lui") != -1)) {
            if (secondAttempt) {
...
function convertAccountsToObjects(str) {
...
async function serializeAccounts(accounts) {
    const allAccountsToSerialize = [];
    ...
    accounts.forEach(account => {
        ...
        accountToSerialize.mails = JSON.stringify(account.getMails());
        accountToSerialize.unsnoozedEmails = JSON.stringify(account.getUnsnoozedEmails());
        accountToSerialize.emailsInAllLabels = JSON.stringify(account.getEmailsInAllLabels());

        allAccountsToSerialize.push(accountToSerialize);
    });

    await storage.set("accounts", allAccountsToSerialize);
}

async function retrieveAccounts() {
    const accounts = await storage.get("accounts");
    const allAccountObjects = [];
    ...
    accounts.forEach(account => {
        ...
        accountObj.setMail(convertMailsToObjects(JSON.parse(account.mails, dateReviver), accountObj));
        accountObj.setUnsnoozedEmails(convertMailsToObjects(JSON.parse(account.unsnoozedEmails, dateReviver), accountObj));
        accountObj.setEmailsInAllLabels(convertMailsToObjects(JSON.parse(account.emailsInAllLabels, dateReviver), accountObj));

        allAccountObjects.push(accountObj);
    });

    return allAccountObjects;
}