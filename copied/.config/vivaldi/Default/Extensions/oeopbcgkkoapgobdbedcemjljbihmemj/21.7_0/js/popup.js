...
async function getBGObjects() {
    ...
    accounts = bg.accounts; //await retrieveAccounts();
			...
			if (mail.messages && mail.messages.last()) {
				...
				$replyArea.find("#send, #sendAndArchive, #sendAndDelete").off().click(function(e) {
					...
					mail.postReply(replyMessageText, replyAllFlag).then(function(response) {
						...
						newMessage.from = {name:getMessage("me"), email:mail.account.getEmail()};
...
function openOtherMail($mail, direction) {
    ...
    const mailAccount = $mail.data("mail").account;
				...
				.one("mousedown", async function() { // MUST use .one because mousedown will also be called when menu items *inside the dropdown all are also clicked
					...
					$account.find(".ignore")
						.click(async function() {
                            ...
                            try {
                                await bg.pollAccounts({showNotification:true});
                                location.reload();
                            } catch (error) {
                                showError(error);
                            } finally {
...
function findMailById(id) {
	...
	accounts.some(function(account) {
		return account.getMails().some(function(mail) {
			if (mail.id == id) {
				foundMail = mail;
				return true;
			}
