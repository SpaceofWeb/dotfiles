...
async function playSound(soundName) {
	...
	await bg.playNotificationSound(soundName);
    playing = false;
...
async function grantPermissionToEmails(tokenResponse) {
	...
	account.fetchSendAs().catch(error => {
		...
		account.getEmails().then(() => {
			loadAccountsOptions({ selectedEmail: account.getEmail() });
			hideLoading();
...
$(document).ready(function() {
	...
	(async () => {
        ...
        function highlightTab(node) {
            ...
            $("paper-tab[value='" + tabName + "']:not(.iron-selected)").addClass("search-result");
        }
        ...
        function search(search) {
            if (!window.initTabsForSearch) {
                document.querySelectorAll("paper-tab").forEach(tab => {
                    initPage(tab.getAttribute("value"));
                });
            ...
            if (search.length >= 2) {
                ...
                for(var ii = 0; ii < len; ii++) {
                    ...
                    if (label && label.toLowerCase().indexOf(search) != -1) {
                    ...
                    for (var jj = 0; jj < len2; jj++) {
                        if (myChildred[jj].nodeType === 3) {
                            if (myChildred[jj].nodeValue.toLowerCase().indexOf(search) != -1) {
        ...
        $("#search").keyup(function(e) {
            ...
            analyticsTimeout = setTimeout(() => {
                if (this.value) {
                    sendGA("optionsSearch", this.value);
                }
            }, 1000);
        });
