...
DetectClient.isChrome = function() {
	return /chrome/i.test(navigator.userAgent) && !DetectClient.isEdge() && !DetectClient.isOpera();
}
...
async function initMisc(params = {}) {
    console.time("initMisc");
    if (!window.calledInitMiscWindowVars) {
        window.calledInitMiscWindowVars = new Date();
        console.info("initWindowVars");
...
function OAuthForDevices(defaultParams) {
	...
	const GOOGLE_CLIENT_SECRET = "CedCVCHth4AolzroefqkhimX";

	const GOOGLE_OLD_CLIENT_ID = "450788627700.apps.googleusercontent.com";
	const GOOGLE_OLD_CLIENT_SECRET = "Wf-ObXmgsyANHeWZD-1AWJ2H";
	
	const GOOGLE_REDIRECT_URI = "https://jasonsavard.com/oauth2callback";

    async function getTokenResponses() {
        return await storage.get(defaultParams.storageKey) || [];
    }

    async function setTokenResponses(tokenResponses) {
        await storage.set(defaultParams.storageKey, tokenResponses);
    }
	...
	async function ensureToken(tokenResponse) {
        if (tokenResponse.chromeProfile) {
            ...
            try {
                ...
                if (error.toString().indexOf("OAuth2 not granted or revoked") != -1) {
                    error.code = 401;
                }
                throw error;
            }
	...
	async function refreshToken(tokenResponse) {
        ...
        try {
            data = await oauthFetch(GOOGLE_TOKEN_URL, data, {method: "post"});
        } catch (error) {
            error.tokenResponse = tokenResponse;
            error.oauthAction = "refreshToken";
            throw error;
	...
	this.getAccessToken = async function(params) {
        ...
        if (params.code) {
            ...
            tokenResponse = await oauthFetch(GOOGLE_TOKEN_URL, {
                code:           params.code,
                client_id:      GOOGLE_CLIENT_ID,
                client_secret:  GOOGLE_CLIENT_SECRET,
